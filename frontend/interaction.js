...
const MODES=[["assistant","عادي"],["plan","تخطيط"],["review","مراجعة"],["fix","إصلاح"],["explain","شرح"],["optimize","تحسين"],["security","أمان"],["test","اختبار"]];
const byId=id=>document.getElementById(id);
const call=(name,...args)=>{const fn=window[name];if(typeof fn!=="function")throw new Error("Aizen action unavailable: "+name);return fn(...args);};
const toast=m=>{try{window.showToast?.(m);}catch{}};
function toggleSidebar(){const s=byId("sidebar");if(!s)return;const mobile=matchMedia("(max-width:800px)").matches;s.classList.toggle(mobile?"open":"aizen-collapsed");document.body.classList.toggle("aizen-sidebar-open",mobile&&s.classList.contains("open"));}
function closeSidebar(){const s=byId("sidebar");if(!s)return;s.classList.remove("open","aizen-collapsed");document.body.classList.remove("aizen-sidebar-open");}
function toggleAccount(){const m=byId("accountMenu");if(!m)return;const opening=m.classList.contains("hidden");m.classList.toggle("hidden",!opening);if(opening)closeSidebar();}
